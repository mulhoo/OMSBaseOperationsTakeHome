...
const Dashboard = () => {
  ...
  const SAN_FRANCISCO_ID = 98;
  const API_BASE = 'http://localhost:3000/api/events';

  useEffect(() => {
    fetchSanFranciscoData();
  }, []);

  const fetchSanFranciscoData = async () => {
    setLoading(true);
    setError(null);

    try {
      const [monthlyResponse, recentResponse] = await Promise.all([
        fetch(`${API_BASE}/monthly/${SAN_FRANCISCO_ID}`),
        fetch(`${API_BASE}/recent/${SAN_FRANCISCO_ID}`)
      ]);

      if (!monthlyResponse.ok || !recentResponse.ok) {
        throw new Error('Failed to fetch San Francisco data');
      }
      ...
      <div className="dashboard-max-width">
        <div className="dashboard-card">
          <h2 className="content-header">
            San Francisco Threat Analysis
          </h2>
          <p className="content-subtitle">
            Monitoring threat events and patterns for San Francisco City
          </p>
        </div>
        ...
        <div className="dashboard-card">
          ...
          {loading ? (
            ...
            <div className="empty-state">
              ...
              <p className="empty-state-title">No monthly data available for San Francisco</p>
        ...
        <div className="dashboard-card">
          <div className="section-header-container">
            ...
            <div className="location-info">
              Last 3 events in San Francisco
            </div>
          </div>
          ...
          {loading ? (
            ...
            <div className="empty-state">
              ...
              <p className="empty-state-title">No recent events found for San Francisco</p>
              <p className="empty-state-description">Check your API connection and try again</p>
            </div>
          )}
        </div>
        <div className="dashboard-footer">
          <p>Base Operations Threat Analysis Dashboard - San Francisco Focus</p>
        </div>
      </div>
    </div>
  );
